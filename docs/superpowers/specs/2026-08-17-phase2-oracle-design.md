...
| 6 | Service-point membership | the manuscript says only "the number of clinics **within** an administrative unit" (Eq. 3 exposition, p. 15) and is **SILENT** on a point lying exactly on a shared boundary; the reference impl resolves that ambiguity as strict containment (`within`) — a defensible reading, not a manuscript-literal rule | production's `add_point_count_column` uses `gpd.sjoin`'s default `intersects`, so a boundary point is counted for BOTH neighbors (verified: a clinic on the A\|B edge gives A=1 AND B=1; the reference impl counts it for neither). **Measured against the real Delhi layers (Aug 2026): this gap is currently LATENT — zero service points lie exactly on a colony boundary in any of the six point layers (closest approach 1.3 mm, transport).** What DOES double-count today is a different mechanism the oracle did not model: the colony layer contains 4,050 overlapping polygon pairs, placing ~450 service points inside two or more colonies (bank 232, ration 104, school 53, transport 41, health 18, police 2) — `within` would not fix any of those. Routed to the Phase 3 bug audit as a separate, higher-priority item. Pinned by `tests/test_oracle.py::test_gap6_border_point_is_double_counted_by_production` |
